...
data AAction
        = ACall {        -- state method call
            ...
            -- the condition under which the call fires
            aact_cond :: AExpr,
            -- one inner list per source-language argument; each inner list
            -- holds the input-port AExprs for that argument
            acall_args :: [[AExpr]]
        }
        | AFCall {       -- foreign function call
            aact_objid :: AId,
            afcall_fun :: String,
            afcall_isC :: Bool,
            -- the condition under which the call fires
...
instance NFData AAction where
    rnf (ACall oid mid cond args) = rnf4 oid mid cond args
    rnf (AFCall oid fun isC cond args assump) =
        rnf6 oid fun isC cond args assump
    rnf (ATaskAction oid fun isC cookie cond args temp vtyp assump) =
        rnf9 oid fun isC cookie cond args temp vtyp assump

-- Uniform view of an AAction's expressions as a flat list with the
-- condition leading.  For ACall this re-flattens the per-argument input
-- port groups.
aActionArgs :: AAction -> [AExpr]
aActionArgs (ACall { aact_cond = c, acall_args = args }) =
    c : concat args
aActionArgs a = aact_cond a : aact_args a
...
data AExpr
        ...
        | AMethCall {
            ...
            ae_objid :: AId,
            ameth_id :: AMethodId,
            -- external state method call: one inner list per source argument
            ame_args :: [[AExpr]]
        }
...
instance PPrint AAction where
    pPrint d _ (ACall i m c args) | isOne c =
        pPrint d 0 i <> text "." <> ppMethId d m <+>
        sep (map (pPrint d 1) (concat args))
    pPrint d _ (ACall i m c args) = sep [
        text "if" <+> pPrint d 0 c <+> text "then",
        nest 2 (pPrint d 0 i <> text "." <> ppMethId d m <+>
                sep (map (pPrint d 1) (concat args)))
        ]
...
--      pparen (p>0) $ sepList (map (pPrint d 1) es) (text "" <+> pPrint d 1 o <> (text ("_" ++ (createPositionString (getIdPosition aid)))))
    ...
    pPrint d p (AMethCall _ i m args) =
        ...
        pPrint d 1 i <>
        sep (text "." <> ppMethId d m : map (pPrint d 1) (concat args))
...
instance PPrintExpand AAction where
    pPrintExpand m d _ (ACall i meth c args) | isOne c =
        pPrint d 0 i <> text "." <> ppMethId d meth <+>
        sep (map (pPrintExpand m d pContext) (concat args))
    pPrintExpand m d _ (ACall i meth c args) = sep [
        text "if" <+> pPrintExpand m d bContext c <+> text "then",
        nest 2 (pPrint d 0 i <> text "." <> ppMethId d meth <+>
                sep (map (pPrintExpand m d pContext) (concat args)))
        ]
