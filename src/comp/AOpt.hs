...
aOptTree bflgs ofunc (AMethCall t i m args) =
    mapM (mapM (aOptTree bflgs ofunc)) args >>= return . AMethCall t i m
...
aInsertCase stringOK findFn (AMethCall t i m args) =
    let args' = map (map (aInsertCase stringOK findFn)) args
...
aExp bflags (AMethCall t i m args) =
    mapM (mapM (aExp bflags)) args >>= return . AMethCall t i m
