...
aSelExpr ((sel, atype) : sels) (ICon i (ICStateVar { }) : es)
  ...
  i' <- transId i
  -- per-source-arg port groups for the AMethCall
  args <- mapM aTupleExpr es
...
-- A source argument whose hardware-port view is empty has type PrimUnit and
-- contributes no entries; this keeps avi_meth_types aligned with vf_inputs.
...
aAction1 _ cond (IAps (ICon m (ICSel { })) _ (ICon i (ICStateVar { }) : es)) = do
        cond' <- aSExpr cond
        -- One inner list per source argument: aTupleExpr flattens a
        -- SplitPorts tuple into its input-port AExprs, returns a singleton
        -- for a normal arg, and returns [] for a PrimUnit (zero-port) arg.
        args <- mapM aTupleExpr es
        i' <- transId i
        return [ACall i' m cond' args]
