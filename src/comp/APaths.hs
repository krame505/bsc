...
mkActionEdges :: PathEnv -> PathNode -> AAction ->
                 [(PathNode, PathNode)]
mkActionEdges env en (ACall state_id qual_meth_id cond args) =
    let meth_id = unQualId qual_meth_id
        ...
        -- Flatten the per-arg port groups while tracking (argN, portM) so
        -- each AExpr maps to the right hardware port.
        argPortPairs =
            [ (PNStateMethodArg state_id meth_id argN portM, e)
            | (argN, ports) <- zip [1..] args
            , (portM, e)    <- zip [1..] ports ]
...
mkActionEdges env en a@(AFCall {}) =
    -- XXX right now, we don't track cycles through function calls
    concatMap (snd3 . findEdges env) (aact_cond a : aact_args a)
mkActionEdges env en a@(ATaskAction {}) =
