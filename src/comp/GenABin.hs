...
instance Bin AAction where
    writeBytes (ACall i m c args) =
        do putI 0; toBin i; toBin m; toBin c; toBin args
    writeBytes (AFCall i f isC c as isA) =
        do putI 1; toBin i; toBin f; toBin isC; toBin c; toBin as; toBin isA
    writeBytes (ATaskAction i f isC n c as tmp ty isA) =
        do putI 2; toBin i; toBin f; toBin isC; toBin n; toBin c; toBin as;
           toBin tmp; toBin ty; toBin isA
    readBytes =
        do i <- getI
           case i of
            0 -> do i <- fromBin; m <- fromBin; c <- fromBin;
                    args <- fromBin;
                    return (ACall i m c args)
            1 -> do i <- fromBin; f <- fromBin; isC <- fromBin;
                    c <- fromBin; as <- fromBin; isA <- fromBin;
                    return (AFCall i f isC c as isA)
            2 -> do i <- fromBin; f <- fromBin; isC <- fromBin; n <- fromBin;
                    c <- fromBin; as <- fromBin; tmp <- fromBin;
                    ty <- fromBin; isA <- fromBin;
                    return (ATaskAction i f isC n c as tmp ty isA)
